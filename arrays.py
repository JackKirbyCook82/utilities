...
# FACTORY
@keydispatcher
def xarray_fromdata(datatype, datakey, data): raise KeyError(datatype)

@xarray_fromdata.register('dataframe')
def _xarray_fromdataframe(datakey, data): 
...
def cumulate(array, *args, direction='upper', **kwargs): 
    return {'upper': lambda x: np.cumsum(x), 'lower': lambda x: np.flip(np.cumsum(np.flip(x, 0)), 0)}[direction](array)
